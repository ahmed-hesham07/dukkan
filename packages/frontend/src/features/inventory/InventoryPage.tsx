...
export default function InventoryPage() {
  ...
  const [newProduct, setNewProduct] = useState({ name: '', price: '', costPrice: '', stock: '0', lowStockThreshold: '5' });
  ...
  const handleAddProduct = async () => {
    ...
    const product: LocalProduct = {
      id, tenantId, name: name.trim(), price: parseFloat(price),
      costPrice: costPrice ? parseFloat(costPrice) : undefined,
      stock: parseInt(stock) || 0, lowStockThreshold: parseInt(lowStockThreshold) || 5,
  ...
  const handleAdjust = async (product: LocalProduct, dir: 1 | -1) => {
    ...
    await localDb.products.where('id').equals(product.id).modify({ stock: Math.max(0, product.stock + finalDelta) });
    await enqueue(uuidv4(), 'stock', 'update', { productId: product.id, delta: finalDelta }, tenantId);
    runSync();
    setAdjustingId(null); setDelta('');
    ...
    <div className="page-container">
      <header className="page-header">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-black" style={{ color: '#130F2A' }}>{t('inventory.title')}</h1>
            {lowStockCount > 0 && (
      ...
      <div className="p-4 space-y-3">
        ...
        {/* Add form */}
        {showAddForm && (
          <div className="rounded-2xl p-4 space-y-3 animate-slide-down"
            style={{ background: '#FFFFFF', border: '1px solid #DDD6FE', boxShadow: '0 4px 16px rgba(124,58,237,0.1)' }}>
            <p className="text-sm font-black" style={{ color: '#130F2A' }}>{t('inventory.add')}</p>
            ...
            <div className="grid grid-cols-2 gap-2">
              <input className="input-field" placeholder={t('inventory.stock')} type="number" inputMode="numeric"
                value={newProduct.stock} onChange={(e) => setNewProduct({ ...newProduct, stock: e.target.value })} />
              <input className="input-field" placeholder={t('inventory.alertLevel')} type="number" inputMode="numeric"
        ...
        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center mt-20 animate-fade-in gap-3">
            <div className="w-20 h-20 rounded-3xl flex items-center justify-center text-4xl"
              style={{ background: '#EDE9FE', border: '1px solid #DDD6FE' }}>📦</div>
            <p className="text-sm font-semibold" style={{ color: '#9C94B8' }}>{t('inventory.noProducts')}</p>
          </div>
        ) : (
          <div className="space-y-2.5 pb-4">
            {products.map((p) => {
              ...
              const margin = p.costPrice && p.price > 0
                ? Math.round(((p.price - p.costPrice) / p.price) * 100) : null;

              return (
                <div key={p.id} className="rounded-2xl overflow-hidden"
                  style={{
                    background: '#FFFFFF',
                    border: isLow ? '1px solid #FECACA' : '1px solid #E8E6F5',
                    boxShadow: isLow ? '0 2px 8px rgba(239,68,68,0.08)' : '0 1px 6px rgba(19,15,42,0.05)',
                  }}>
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="font-bold" style={{ color: '#130F2A' }}>{p.name}</p>
                        <p className="text-xl font-black mt-1" style={{ color: '#7C3AED' }}>
                          {p.price.toFixed(2)}
                        ...
                        {p.costPrice !== undefined && p.costPrice !== null && (
                          <p className="text-xs mt-0.5 font-medium" style={{ color: '#9C94B8' }}>
                            {t('inventory.costPrice')}: {p.costPrice.toFixed(2)}
                            {margin !== null && (
                              <span className="ms-2 font-black"
                                style={{ color: margin >= 0 ? '#059669' : '#DC2626' }}>
                      ...
                      <div className="text-end flex-shrink-0">
                        <span className="text-3xl font-black block"
                          style={{ color: isLow ? '#DC2626' : '#059669' }}>{p.stock}</span>
                        {isLow && (
                    ...
                    {isAdjusting ? (
                      <div className="mt-4 space-y-3 animate-slide-down">
                        <input className="input-field text-center text-xl font-bold" placeholder={t('inventory.amount')}
                          type="number" inputMode="numeric" min="1"
                          value={delta} onChange={(e) => setDelta(e.target.value)} autoFocus />
                        <div className="grid grid-cols-2 gap-2">
                          <button className="font-bold rounded-xl py-3.5 text-sm text-white active:scale-95 transition-all"
                        ...
                        <button className="btn-secondary py-2.5 text-sm" onClick={() => { setAdjustingId(null); setDelta(''); }}>
                          {t('inventory.cancelAdjust')}
                        </button>
                      </div>
                    ) : (
                      <button className="mt-3 w-full rounded-xl py-2.5 text-sm font-semibold transition-all active:scale-95"
                        style={{ background: '#F5F4FF', border: '1px solid #E2DFF0', color: '#7C3AED' }}
